		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					//Particle color
					fixed4 particleColor = lerp(_StartColor, _EndColor, i.color.r);

					//Noise Layer UVs
					float2 UVBaseLayer = ((i.uv + float2(_OffsetU, _OffsetV)) + (_Time.x * float2(_ScrollSpeedU, _ScrollSpeedV) * i.color.g)) * _Scale * i.color.b;
					float2 UVLayer2 = ((i.uv + float2(_OffsetULayer2, _OffsetVLayer2)) + (_Time.x * float2(_ScrollSpeedULayer2, _ScrollSpeedVLayer2) * i.color.g)) * _ScaleMultiplierLayer2 * i.color.b;
					float2 UVLayer3 = ((i.uv + float2(_OffsetULayer3, _OffsetVLayer3)) + (_Time.x * float2(_ScrollSpeedULayer3, _ScrollSpeedVLayer3) * i.color.g)) * _ScaleMultiplierLayer3 * i.color.b;
					...
					fixed3 color = noiseTex.rgb * _MainColor.rgb * _ColorMultiplier;
					fixed3 colorLayer2 = noiseTexLayer2.rgb * _MainColor.rgb;
					fixed3 colorLayer3 = noiseTexLayer3.rgb * _MainColor.rgb;

					fixed noise = noiseTex.a * _AlphaMultiplier;
					fixed noiseLayer2 = noiseTexLayer2.a;
					fixed noiseLayer3 = noiseTexLayer3.a;

					//return float4(color, noise);
					...
					//Combine
					fixed4 col;
					col.rgb = ((color * colorLayer2) * colorLayer3) * particleColor.rgb;
